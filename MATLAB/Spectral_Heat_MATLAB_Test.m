...
% Define variables from Data Generation file and exact solution
...
f = @(r,z,th,t) 10*exp(-((pi*q1)^2 + w33^2)*t)*sin(q1*pi*z).*besselj(3,w33*r).*sin(3*th) + ... 
...
disp('CFS (only first layer is nonzero): ');
disp(CFS);
...
disp('CFS (layers 1-4 are nonzero): ');
disp(CFS)

%% Full Time Step
q = (12/25)*alph;
disp('STARTING FULL TIME STEP')
for d = 5:T
    RHS = (48/25)*CFS(:,:,:,d-1)-(36/25)*CFS(:,:,:,d-2)+ ... 
        (16/25)*CFS(:,:,:,d-3)-(3/25)*CFS(:,:,:,d-4);
